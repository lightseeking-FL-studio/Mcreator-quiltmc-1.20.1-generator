...
<#-- Vanilla 1.20.1 does not have makesPiglinsNeutral on Item. Left empty for Quilt. -->
</#macro>
...
<#if hasProcedure(procedure)>
@Override @Environment(EnvType.CLIENT) public void animateTick(BlockState blockstate, Level world, BlockPos pos, RandomSource random) {
...
<#if hasProcedure(procedure)>
@Override public void playerWillDestroy(Level world, BlockPos pos, BlockState blockstate, Player entity) {
	super.playerWillDestroy(world, pos, blockstate, entity);
	<@procedureCode procedure, {
	"x": "pos.getX()",
	"y": "pos.getY()",
	"z": "pos.getZ()",
