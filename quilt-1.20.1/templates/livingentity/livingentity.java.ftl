...
public class ${name}Entity extends ${extendsClass} <#if interfaces?size gt 0>implements ${interfaces?join(",")}</#if> {
	...
	private final java.util.Set<java.util.UUID> th$recentLightningHits = new java.util.HashSet<>();

	@Override public void thunderHit(ServerLevel serverWorld, LightningBolt lightningBolt) {
		super.thunderHit(serverWorld, lightningBolt);
		if (lightningBolt != null && th$recentLightningHits.add(lightningBolt.getUUID())) {
	...
	private final NonNullList<ItemStack> inventory = NonNullList.withSize(${data.inventorySize}, ItemStack.EMPTY);
	...
	@Override public void addAdditionalSaveData(CompoundTag compound) {
		...
		<#if data.guiBoundTo?has_content>
		compound.put("InventoryCustom", ContainerHelper.saveAllItems(new CompoundTag(), this.inventory));
		</#if>
		<#if data.sensitiveToVibration>
	...
	@Override public void readAdditionalSaveData(CompoundTag compound) {
		...
		if (compound.contains("listener", Tag.TAG_COMPOUND)) {
			VibrationSystem.Data.CODEC.parse(new com.mojang.serialization.Dynamic<>(net.minecraft.nbt.NbtOps.INSTANCE, compound.getCompound("listener")))
				...
				if (sourceentity instanceof ServerPlayer serverPlayer) {
					serverPlayer.openMenu(new MenuProvider() {
						...
						@Override public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
							FriendlyByteBuf packetBuffer = new FriendlyByteBuf(io.netty.buffer.Unpooled.buffer());
	...
	public static void init() {
		<#if data.spawnThisMob>
			...
			SpawnPlacements.register(${JavaModName}Entities.${REGISTRYNAME},
		...
		<#if data.spawnInDungeons>
			// Dungeon spawning is handled by Minecraft's built-in dungeon generation
			// DungeonHooks is a Forge-only API and is not available in Fabric/Quilt
		</#if>

		<#if data.mobBehaviourType == "Raider">
		Raid.RaiderType.create("${registryname}", ${JavaModName}Entities.${REGISTRYNAME}, new int[]{0, ${data.raidSpawnsCount[0]}, ${data.raidSpawnsCount[1]}, ${data.raidSpawnsCount[2]}, ${data.raidSpawnsCount[3]}, ${data.raidSpawnsCount[4]}, ${data.raidSpawnsCount[5]}, ${data.raidSpawnsCount[6]}});
		</#if>
	}
