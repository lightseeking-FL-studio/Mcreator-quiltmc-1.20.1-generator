...
public class ${name}Procedure {
	<#if trigger_code?has_content>public static boolean eventResult = true;</#if>
...
${trigger_code}
	...
	<#if trigger_code?has_content>
		...
		<#else>
			public static <#if return_type??>${return_type.getJavaType(generator.getWorkspace())}<#else>void</#if> execute() {
				<#if return_type??>return </#if>execute((Object) null);
			}
		</#if>
	</#if>
	...
	public static <#if return_type??>${return_type.getJavaType(generator.getWorkspace())}<#else>void</#if> execute(Map<String, Object> dependencies) {
		<#if return_type??>return </#if>execute(<#if trigger_code?has_content><#if dependencies?has_content>null, </#if><#list dependencies as dependency><#if dependency.getRawType() == "number">((Number) dependencies.get("${dependency.getName()}")).doubleValue()<#else>(${dependency.getType(generator.getWorkspace())}) dependencies.get("${dependency.getName()}")</#if><#sep>, </#list><#if trigger_code?has_content && !dependencies?has_content>(Object) null</#if><#else><#if dependencies?has_content><#list dependencies as dependency><#if dependency.getRawType() == "number">((Number) dependencies.get("${dependency.getName()}")).doubleValue()<#else>(${dependency.getType(generator.getWorkspace())}) dependencies.get("${dependency.getName()}")</#if><#sep>, </#list><#else>null</#if></#if>);
	}
	...
	<#if trigger_code?has_content>private <#else>public </#if>static <#if return_type??>${return_type.getJavaType(generator.getWorkspace())}<#else>void</#if> execute(
		<#if trigger_code?has_content>@Nullable Object event<#if dependencies?has_content>,</#if></#if>
		<#list dependencies as dependency>
				${dependency.getType(generator.getWorkspace())} ${dependency.getName()}<#sep>,
		</#list>
	) {
