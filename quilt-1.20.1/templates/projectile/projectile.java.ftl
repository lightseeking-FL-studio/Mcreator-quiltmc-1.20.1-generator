...
public class ${name}Entity extends AbstractArrow implements ItemSupplier {
	...
	public ${name}Entity(EntityType<? extends AbstractArrow> type, Level world) {
		super(type, world);
	}

	public ${name}Entity(EntityType<? extends AbstractArrow> type, LivingEntity entity, Level world) {
		super(type, entity, world);
	}

	public ${name}Entity(EntityType<? extends AbstractArrow> type, double x, double y, double z, Level world) {
		super(type, x, y, z, world);
	}

	@Override public ItemStack getPickupItem() {
	...
	<#if hasProcedure(data.onHitsEntity) || hasProcedure(data.onHitsPlayer)>
	...
	public static ${name}Entity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		${name}Entity entityarrow = new ${name}Entity(${JavaModName}Entities.${REGISTRYNAME}, entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.ARROW_SHOOT, SoundSource.PLAYERS, 1,
	...
	public static ${name}Entity shoot(LivingEntity entity, LivingEntity target) {
		${name}Entity entityarrow = new ${name}Entity(${JavaModName}Entities.${REGISTRYNAME}, entity, entity.level());
		...
		<#if data.actionSound.toString()?has_content>
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.ARROW_SHOOT, SoundSource.PLAYERS, 1,
