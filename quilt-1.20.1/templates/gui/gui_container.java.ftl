...
public class ${name}Menu extends AbstractContainerMenu implements ${JavaModName}Menus.MenuAccessor {
	...
	private final SimpleContainer internal = new SimpleContainer(${data.getMaxSlotID() + 1});
		...
		<#if data.type == 1>
			...
			<#list data.components as component>
				<#if component.getClass().getSimpleName()?ends_with("Slot")>
					<#assign slotnum += 1>
					this.customSlots.put(${component.id}, this.addSlot(new Slot(internal, ${component.id},
					...
					@Override public void setChanged() {
						super.setChanged();
						if (!ItemStack.matches(lastStack, this.getItem())) {
							lastStack = this.getItem().copy();
	...
	<#if data.type == 1>
		...
		<#-- #47997 -->
		@Override protected boolean moveItemStackTo(ItemStack itemstack, int index, int endIndex, boolean reverseDirection) {
			boolean flag = false;
			int i = index;
			if (reverseDirection) {
				i = endIndex - 1;
			...
			while (!itemstack.isEmpty()) {
				...
				if (!itemstack1.isEmpty() && ItemStack.isSameItemSameTags(itemstack, itemstack1) && slot.mayPlace(itemstack)) {
					int j = Math.min(itemstack1.getMaxStackSize() - itemstack1.getCount(), itemstack.getCount());
					int k = Math.min(slot.getMaxStackSize(itemstack1) - itemstack1.getCount(), j);
					if (k > 0) {
						itemstack1.shrink(k);
						itemstack.shrink(k);
						slot.set(itemstack1);
			...
			if (!itemstack.isEmpty()) {
				...
				while (!itemstack.isEmpty()) {
					...
					if (itemstack1.isEmpty() && slot.mayPlace(itemstack)) {
						int l = Math.min(slot.getMaxStackSize(itemstack), itemstack.getCount());
						slot.setByPlayer(itemstack.split(l));
						slot.setChanged();
						flag = true;
						break;
					}
			...
			if (!bound && playerIn instanceof ServerPlayer serverPlayer) {
				if (!serverPlayer.isAlive() || serverPlayer.hasDisconnected()) {
					for(int j = 0; j < internal.getContainerSize(); ++j) {
						...
						SimpleContainer container = internal;
 					}
 				} else {
		...
		<#if data.hasSlotEvents()>
			private void slotChanged(int slotid, int ctype, int meta) {
				if(this.world != null && this.world.isClientSide()) {
					net.minecraft.network.FriendlyByteBuf slotBuf = PacketByteBufs.create();
					${name}SlotMessage.buffer(new ${name}SlotMessage(slotid, x, y, z, ctype, meta), slotBuf);
					org.quiltmc.qsl.networking.api.client.ClientPlayNetworking.send(${name}SlotMessage.ID, slotBuf);
				}
			}
		</#if>
	<#else>
	...
	public static void onContainerOpen(Player entity, AbstractContainerMenu container) {
		if(container instanceof ${name}Menu menu) {
			Level world = menu.world;
			double x = menu.x;
			double y = menu.y;
			double z = menu.z;
