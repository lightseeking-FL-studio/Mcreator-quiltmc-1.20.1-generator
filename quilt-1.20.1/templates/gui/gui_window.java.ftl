...
public class ${name}Screen extends AbstractContainerScreen<${name}Menu> implements ${JavaModName}Screens.ScreenAccessor {
	...
	private AbstractSliderButton ${component.getName()};
	</#list>
	...
	@Override public void init() {
		...
		<#assign slid = 0>
		<#list sliders as component>
			${component.getName()} = new AbstractSliderButton(this.leftPos + ${component.gx(data.width)}, this.topPos + ${component.gy(data.height)},
				${component.getWidth(w.getWorkspace())}, ${component.getHeight(w.getWorkspace())},
				Component.translatable("gui.${modid}.${registryname}.${component.getName()}_prefix"),
				(${component.value} - ${component.min}) / (${component.max} - ${component.min})) {
					private double minVal = ${component.min};
					private double maxVal = ${component.max};
					private double stepVal = ${component.step};
					@Override protected void updateMessage() {
						this.setMessage(Component.translatable("gui.${modid}.${registryname}.${component.getName()}_prefix")
							.append(Component.literal(": " + String.format("%.1f", minVal + (maxVal - minVal) * this.value))));
					}
					@Override protected void applyValue() {
						if (!menuStateUpdateActive)
							menu.sendMenuStateUpdate(entity, 2, "${component.getName()}", minVal + (maxVal - minVal) * this.value, false);
						<#if hasProcedure(component.whenSliderMoves)>
							net.minecraft.network.FriendlyByteBuf sliderBuf = PacketByteBufs.create();
							${name}SliderMessage.buffer(new ${name}SliderMessage(${slid}, x, y, z, minVal + (maxVal - minVal) * this.value), sliderBuf);
							org.quiltmc.qsl.networking.api.client.ClientPlayNetworking.send(${name}SliderMessage.ID, sliderBuf);
						</#if>
					}
					public double getValue() {
						return minVal + (maxVal - minVal) * this.value;
					}
					public void setValue(double v) {
						this.value = (v - minVal) / (maxVal - minVal);
						updateMessage();
					}
				};
			this.addRenderableWidget(${component.getName()});
...
e -> {
	<#if hasProcedure(component.onClick)>
		...
		if (<@procedureOBJToConditionCode component.displayCondition/>) {
			net.minecraft.network.FriendlyByteBuf buttonBuf = PacketByteBufs.create();
			${name}ButtonMessage.buffer(new ${name}ButtonMessage(${btid}, x, y, z), buttonBuf);
			org.quiltmc.qsl.networking.api.client.ClientPlayNetworking.send(${name}ButtonMessage.ID, buttonBuf);
		}
	</#if>
}
</#macro>
