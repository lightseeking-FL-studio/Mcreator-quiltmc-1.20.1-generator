...
public class MenuStateUpdateMessage {
	...
	public static final ResourceLocation ID = new ResourceLocation("${modid}", "menu_state_update");
    ...
    public static void registerOnServer() {
		ServerPlayNetworking.registerGlobalReceiver(ID, (server, player, handler, buf, responseSender) -> {
			MenuStateUpdateMessage message = new MenuStateUpdateMessage(buf);
			server.execute(() -> {
				if (message.name.length() > 256 || message.elementState instanceof String string && string.length() > 8192)
					return;
				if (player.containerMenu instanceof ${JavaModName}Menus.MenuAccessor menu) {
	...
	public static void registerOnClient() {
		ClientPlayNetworking.registerGlobalReceiver(ID, (client, handler, buf, responseSender) -> {
			...
			client.execute(() -> {
				if (message.name.length() > 256 || message.elementState instanceof String string && string.length() > 8192)
					return;
				if (client.player != null && client.player.containerMenu instanceof ${JavaModName}Menus.MenuAccessor menu) {
					menu.getMenuState().put(message.elementType + ":" + message.name, message.elementState);
