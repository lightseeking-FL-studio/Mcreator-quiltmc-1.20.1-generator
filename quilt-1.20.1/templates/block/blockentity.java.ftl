...
public class ${name}BlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer
	...
	@Override public void load(CompoundTag compound) {
		...
		<#if data.isFluidTank>
		this.storedFluid = net.minecraft.core.registries.BuiltInRegistries.FLUID.get(new ResourceLocation(compound.getString("fluid")));
	...
	@Override public void saveAdditional(CompoundTag compound) {
		...
		<#if data.hasEnergyStorage>
		compound.putLong("energyStored", this.energyStored);
		</#if>

		<#if data.isFluidTank>
		compound.putString("fluid", net.minecraft.core.registries.BuiltInRegistries.FLUID.getKey(this.storedFluid).toString());
	...
	@Override public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		...
		<#else>
		return new ${data.guiBoundTo}Menu(id, inventory, new FriendlyByteBuf(io.netty.buffer.Unpooled.buffer()).writeBlockPos(this.worldPosition));
		</#if>
	}
	...
	<#if data.hasEnergyStorage>
	private long energyStored = ${data.energyInitial};
	private static final long ENERGY_CAPACITY = ${data.energyCapacity};
	private static final long MAX_RECEIVE = ${data.energyMaxReceive};
	private static final long MAX_EXTRACT = ${data.energyMaxExtract};
    </#if>
